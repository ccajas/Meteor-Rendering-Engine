...
float4 PixelShaderFunction(PixelShaderInput input) : COLOR
{
	...
	*/
		...
		(saturate(ProjectedTexCoords.y) == ProjectedTexCoords.y) && (mask > 0.5))
	{
		...
		float moments = tex2D(ShadowMapSampler, ProjectedTexCoords);
		...
		float fDarkeningFactor = 2;
		float coefficient = fDarkeningFactor * (moments - input.RealDistance.x);
		float3 sum = clamp(exp(coefficient), 0.0, 1.0 );
		return float4(sum, 1);
